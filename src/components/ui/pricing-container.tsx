...
const PricingHeader = ({
  ...
  title: string;
    ...
    y: 0
            ...
            <motion.div className="h-2 bg-gradient-to-r from-black via-gray-600 to-black rounded-full" initial={{
      scaleX: 0
    }} animate={{
      scaleX: 1
    }} transition={{
...
const BackgroundEffects = () => <>
        <div className="absolute inset-0">
            {[...Array(30)].map((_, i) => <motion.div key={i} className="absolute w-2 h-2 bg-black/5 rounded-full" style={{
...
const PricingCard = ({
  ...
  return <motion.div ref={cardRef} key={plan.name} initial={{
    opacity: 0,
    y: 20
  }} animate={{
    opacity: 1,
    ...
    mouseY.set(0);
            ...
            {/* Price Badge */}
            <motion.div className={cn(`absolute -top-4 -right-4 w-16 h-16 
                    rounded-full flex items-center justify-center border-2 border-black
                    shadow-[3px_3px_0px_0px_rgba(0,0,0,0.9)]`, plan.accent)} animate={{
      rotate: [0, 10, 0, -10, 0],
      ...
      ease: [0.76, 0, 0.24, 1]
                ...
                <div className="text-center text-white">
                    <div className="text-[8px] font-bold mb-1">a partir de</div>
            ...
            <div className="space-y-2 mb-4">
                {plan.features.map((feature, i) => <motion.div key={feature} initial={{
        opacity: 0,
        x: -20
      }} animate={{
        opacity: 1,
        ...
        transition: {
          ...
          rotate: 360
            ...
            {/* CTA Button */}
            <motion.button className={cn(`w-full py-2 rounded-lg  text-white font-black text-sm
                    ...
                    transition-all duration-200`, plan.accent)} whileHover={{
      scale: 1.02,
      transition: {
        duration: 0.2
      }
      ...
      scale: 0.95,
      rotate: [-1, 1, 0]
    }} onClick={() => window.open('https://api.whatsapp.com/message/LIGXZ25QRZOME1?autoload=1&app_absent=0', '_blank')}>
                CONTRATAR AGORA →
            </motion.button>
        </motion.div>;
};
...
export const PricingContainer = ({
  ...
  return <div className={`min-h-screen bg-[#f0f0f0] p-4 sm:p-6 lg:p-8 relative overflow-hidden rounded-[12px] ${className}`}>
            <PricingHeader title={title} />
            <BackgroundEffects />

            <div className="w-[100%] max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
